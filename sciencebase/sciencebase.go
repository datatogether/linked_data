...
// A ScienceBase Item is the basic unit of data within the ScienceBase Catalog.
// An Item can be anything from a metadata description of a data service to a
// dataset in the Digital Repository to a description of a project.
// Items share a common set of simple metadata elements based on the
// Dublin Core Metadata Element Set and may be extended with additional properties.
type Item struct {
...
func (i *Item) ChildrenJsonUrl() string {
	return fmt.Sprintf("https://www.sciencebase.gov/catalog/items?parentId=%s&format=json", i.Id)
}
